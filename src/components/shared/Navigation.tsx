...
function NavLink({
  ...
  return (
    <Link
      to={href}
      className={`hover:text-red-400 transition uppercase tracking-widest text-xs font-bold ${active ? "text-red-400" : ""}`}
    >
      {children}
    </Link>
  );
...
function DropdownMenu({
  ...
  return (
    ...
    >
      <button className="hover:text-red-400 transition uppercase tracking-widest flex items-center gap-1 text-xs font-bold">
        {title}
        <ChevronDown
      ...
      <AnimatePresence>
        {open && (
          ...
          >
            <div className="bg-red-950 border border-white/10 rounded-2xl overflow-hidden shadow-2xl py-2">
              {links.map((link, index) => (
                <Link
                  key={link.href}
                  to={link.href}
                  className={`block w-full text-left px-5 py-2.5 text-xs font-bold text-red-100/70 hover:bg-red-500 hover:text-red-950 transition-colors ${index !== links.length - 1 ? "border-b border-white/5" : ""}`}
                >
                  {link.label}
                </Link>
              ))}
...
export default function Navigation() {
  ...
  return (
    <nav className="fixed top-3 left-0 right-0 z-50 px-4" id="main-nav">
      <div
        className={`max-w-5xl mx-auto bg-red-950/90 backdrop-blur-xl border border-white/10 rounded-full h-16 flex items-center justify-between px-6 pr-4 pl-2 shadow-2xl shadow-red-950/20 transition-all duration-300 ${scrolled ? "shadow-red-950/40" : ""}`}
      >
        ...
        <div className="hidden md:flex items-center gap-5 text-red-100/70">
      ...
      <AnimatePresence>
        {mobileOpen && (
          ...
          >
            <div className="bg-red-900 rounded-2xl border border-white/10 p-6 flex flex-col gap-4 shadow-2xl">
              {[
                { href: "/", label: "HOME" },
              ...
              <div className="border-t border-white/10 pt-4">
                <span className="text-[10px] font-black text-red-500 uppercase tracking-[0.2em] mb-2 block">
                  Author Info
                </span>
