...
export default function Navigation() {
  ...
  return (
    <nav className="fixed top-6 left-0 right-0 z-50 px-4" id="main-nav">
      <div
        className={`max-w-7xl mx-auto bg-emerald-950/90 backdrop-blur-xl border border-white/10 rounded-full h-16 flex items-center justify-between px-6 pr-4 shadow-2xl shadow-emerald-950/20 transition-all duration-300 ${scrolled ? 'shadow-emerald-950/40' : ''}`}
      >
        ...
        <div className="hidden md:flex items-center gap-5 text-emerald-100/70">
          ...
          <NavLink href="/venue" active={location.pathname === '/venue'}>Venue</NavLink>
          <NavLink href="/committee" active={location.pathname === '/committee'}>Committee</NavLink>
          <NavLink href="/registration" active={location.pathname === '/registration'}>Registrations</NavLink>
          ...
          >
            <div className="bg-emerald-900 rounded-2xl border border-white/10 p-6 flex flex-col gap-4 shadow-2xl">
              {[
                ...
                <Link key={link.href} to={link.href} className="text-white font-bold text-sm tracking-widest">
                  {link.label}
                </Link>
              ))}
              <div className="border-t border-white/10 pt-4">
                ...
                <div className="flex flex-col gap-3">
                  {authorLinks.map(link => (
                    <Link key={link.href} to={link.href} className="text-white/70 font-bold text-sm tracking-widest">
                      {link.label}
                    </Link>
                  ))}
                </div>
